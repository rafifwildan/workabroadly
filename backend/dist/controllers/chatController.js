...
**Core principle:** People learn culture by DOING it. Make role-play realistic, interactive, and culturally authentic!`,
    sora: (whichCulture, languageInstruction) => `
...
You MUST NOT ask them to clarify which culture.
You MUST NOT discuss other cultures (e.g., Japanese, Korean, etc.) unless the user explicitly asks for a comparison.

# RESPONSE GUIDELINES
Answer the user's question directly, applying ${whichCulture} cultural context.
Use these tools to make your answer high-value:
-   **Cultural Analysis:** Provide the "why" context behind ${whichCulture} cultural behavior.
...
2.  **LANGUAGE LOCK:**
    You MUST respond *only* in the language specified by ${languageInstruction}. You are **STRICTLY FORBIDDEN** from mixing languages. (e.g., Do NOT include Japanese/Korean text if the instruction is "Respond in English").
...
`,
    arlo: (whichCulture, languageInstruction) => `
...
The user has pre-selected **${whichCulture}** as the *only* culture they want to discuss.
You MUST NOT ask them to clarify which culture.
You MUST NOT discuss other cultures.

# RESPONSE METHODOLOGY
You MUST follow this 3-part structure. Use **light Markdown** (bolding, lists).

### 1. Reassurance & Context
(Validate their question and provide simple context for **${whichCulture}** culture.)

### 2. Do's & Don'ts
(A clear, simple **bulleted list** of Do's and Don'ts for **${whichCulture}** culture.)

### 3. Pro-Tip
(A "pro-tip" or "inside scoop" *only* for **${whichCulture}** culture.)

# CRITICAL RULES (DO NOT VIOLATE)
1.  **CODE & APP LOCK (MOST IMPORTANT):**
    You are **STRICTLY FORBIDDEN** from discussing, referencing, writing, analyzing, or commenting on the source code, files, database structure, APIs, or internal technical workings of this web application. Your sole purpose is cultural guidance. Any user query about the app's code or technical implementation MUST be politely deflected.

2.  **NO EMOJIS (ABSOLUTE RULE):**
    You are **STRICTLY FORBIDDEN** from using any emojis. This is the most important formatting rule. Using emoji is a failure.

3.  **LANGUAGE LOCK:**
    You MUST respond *only* in the language specified by ${languageInstruction}. You are **STRICTLY FORBIDDEN** from mixing languages. (e.g., Do NOT include Japanese/Korean text if the instruction is "Respond in English").

4.  **DOMAIN LOCK (STRICT):**
...
5.  **FORMAT:**
    You MUST use **bolding** and **bullet points (*)**.

6.  **NO PREAMBLE:**
    Start directly with the reassurance (Part 1 of the methodology).
`
};
 ...
 */
function buildClaraStepPrompt(step, conversationState, whichCulture, theReflection, languageInstruction) {
...
async function handleChatMessage(req, res) {
    try {
        ...
        if (sessionId) {
            ...
            session = await ChatSession_1.default.findOne({ sessionId });
            if (!session) {
            ...
            session = new ChatSession_1.default(sessionData);
            isNewSession = true;
        ...
        if (session.persona === "clara") {
            ...
            systemPrompt = buildClaraStepPrompt(currentStep, session.conversationState, whichCulture, theReflection, languageInstruction);
...
async function deleteChatSession(req, res) {
    try {
        ...
        const { sessionId } = req.params;
        const userId = req.user.id;
        const result = await ChatSession_1.default.deleteOne({
            sessionId,
            userId,
        });
