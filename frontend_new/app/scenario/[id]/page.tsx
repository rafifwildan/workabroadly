...
export default function ScenarioBriefPage() {
  ...
  useEffect(() => {
    async function fetchScenario() {
      try {
        const data = await ScenariosAPI.get(id as string);
        setScenario(data);
      } catch (e: any) {
  ...
  const cultureLabel =
    scenario.culture === "japanese"
      ? "Japan"
      : scenario.culture === "korean"
      ? "Korea"
      : "Cultural Scenario";
    ...
    <div className="min-h-screen bg-white flex flex-col">
      <div className="flex-1">
        ...
        <div className="max-w-4xl mx-auto px-4 py-8 space-y-6 pb-32">
          ...
          {/* Overview / Context */}
          <div className="border border-gray-200 rounded-lg bg-white shadow-sm p-8 space-y-4">
            <div className="flex items-center gap-3 mb-2">
              <FileText className="w-6 h-6 text-gray-900" />
              <h2 className="text-xl font-bold text-gray-900">Scenario Overview</h2>
            </div>
            <p className="text-base text-gray-600">{brief.overview}</p>

            <h3 className="text-lg font-semibold mt-4">Context</h3>
            <p className="text-base text-gray-600">{brief.context}</p>
          </div>

          {/* Cultural Tips */}
          {brief.culturalTips && brief.culturalTips.length > 0 && (
            <div className="border border-gray-200 rounded-lg bg-white shadow-sm p-8 space-y-3">
              <div className="flex items-center gap-3 mb-2">
              ...
              <ul className="list-disc list-inside text-base text-gray-600">
                {brief.culturalTips.map((tip: string, idx: number) => (
                  <li key={idx}>{tip}</li>
                ))}
              </ul>
            </div>
          ...
          {/* Learning Objectives */}
          {brief.learningObjectives && brief.learningObjectives.length > 0 && (
            <div className="border border-gray-200 rounded-lg bg-white shadow-sm p-8 space-y-3">
              <div className="flex items-center gap-3 mb-2">
              ...
              <ul className="list-disc list-inside text-base text-gray-600">
                {brief.learningObjectives.map((obj: string, idx: number) => (
                  <li key={idx}>{obj}</li>
                ))}
              </ul>
            </div>
